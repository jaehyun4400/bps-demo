...
## v1.3.0 다음 작업 계획 (2026-06-19 확정)

> 사용자가 "프리뷰" 라고 입력하면 아래 내용을 그대로 출력할 것.

### 진행 순서
1. **A. 출석 체크 시스템** → 완료 후
2. **B. 혜귀인 픽업 배너** (신규 캐릭터 없음, 기존 혜귀인 활용)
...
| 수령 조건 | 날짜 기준 1일 1회 (자정 기준 리셋) |
| 진입 경로 | 로비 하단 출석 버튼 → 팝업 |
...
│  골드  다이아  골드  다이아  골드 다이아  다이아 │
│ 1,000   5   3,000   10   5,000  20    50  │
├─────────────────────────────┤
│       [ 오늘 보상 수령 ]      │
└─────────────────────────────┘
✓ = 수령 완료(dim)  ► = 오늘 수령 가능(강조)  🎁 = 대보상
```

**State 구조**
```javascript
checkInDay: 0,       // 현재 사이클 완료 일수 (0~7)
lastCheckIn: '',     // 마지막 수령 날짜 'YYYY-MM-DD'
```
- 수령 가능 조건: `오늘 날짜 !== lastCheckIn`
- 수령 시: 보상 지급 → lastCheckIn = 오늘 → checkInDay++
- Day 7 완료 시: checkInDay = 0 (사이클 리셋)
...
| 경계값 | Day 7 수령 후 Day 1으로 사이클 리셋 확인 |
| 기능 | 당일 수령 후 버튼 비활성화 (중복 방지) |
| 기능 | 자정 이후 날짜 변경 시 수령 버튼 활성화 |
| 기능 | 각 Day별 보상 정확성 확인 (Day 1~7) |
| 기능 | 수령 완료 슬롯 dim 처리 확인 |
| 기능 | 사이클 리셋 후 Day 1 보상 재지급 확인 |
| 회귀 | 출석 수령 후 기존 재화 정상 합산 확인 |

**의도적 버그 — BUG-003 (Major)**
- Day 7 수령 후 checkInDay 0 리셋 코드 누락 → checkInDay=8로 고착
- 결과: 다음 사이클 진입 불가, 출석 버튼 영구 비활성화
